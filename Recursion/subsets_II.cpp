...
{
public:
    void permute(vector<vector<int>> &op, vector<int> curr, vector<int> nums, int start)
    {
        // Pushing currently form subset into output
        op.push_back(curr);
        for (int i = start; i < nums.size(); i++)
        {
            // To acvoid duplicates
            // eg [1,2,2,2,3,3]
            // if start=index-0
            // Means we can pick 2 only once
            // if start=index-1
            // Means for this index wecan pick 2 although curr has 2 in it, but
            // this 2 is different, this is taien care by i!=start condition
            if (i != start && nums[i] == nums[i - 1])
                continue;
            curr.push_back(nums[i]);
            permute(op, curr, nums, i + 1);
            curr.pop_back();
        }
    }
